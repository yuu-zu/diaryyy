...
router.post('/profile', requireSession, async (req, res) => {
  try {
    const fullName = String(req.body.fullName || '').trim();
    ...
    await updateUserProfile(req.session.userId, fullName, birthDate, gender);
...
router.post('/request-account-deletion', requireSession, async (req, res) => {
  try {
    const password = String(req.body.password || '');
    ...
    await setAccountDeletionOtp(req.session.userId, otp, expiresAt);
...
router.post('/confirm-account-deletion', requireSession, async (req, res) => {
  try {
    ...
    await scheduleAccountDeletion(req.session.userId);
    req.session.destroy(() => {
...
router.post('/cancel-account-deletion', requireSession, async (req, res) => {
  try {
    await restoreDeletedAccount(req.session.userId);
